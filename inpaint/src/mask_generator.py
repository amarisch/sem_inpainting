...
def load_mask(flags,id = 0):
    if flags.mask_type == 'random':
        path_mask = os.path.join(PATH_MASKS + '_{}'.format(flags.mask_type),'mask_{}.png'.format(id))
    else:
        path_mask = os.path.join(PATH_MASKS + '_{}'.format(flags.mask_type),'mask_0.png')

    if not os.path.exists(path_mask):
        raise ValueError('Mask not loaded correctly')
    else:
        mask = (cv2.imread(path_mask)[:,:,0]/255).reshape(1,64,64)
    return mask
