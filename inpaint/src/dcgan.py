...
class DCGAN(object):
    def __init__(self, sess, flags, image_size, input = None):
        self.sess = sess
        self.flags = flags
        ...
        self._build_net(input)
        self._tensorboard()
        print("Initialized DCGAN SUCCESS!")

    def build_inpainting_discriminator(self, inp_image):
        d_inpainted, d_logit_inpainted = self.discriminator(inp_image, is_reuse=True)
        d_loss_inpainted = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                            logits=d_logit_inpainted,labels=tf.ones_like(d_logit_inpainted)))
        self.d_loss_inpainted = d_loss_inpainted
    ...
    def _build_net(self, input):
        ...
        if input == None:
            ...
            self.g_samples = self.generator(input)
