...
class Cars(object):
    def __init__(self, flags, dataset_name):
        ...
        self.dataset_name = dataset_name
        self.image_size = (64, 64, 3)
        self.input_height = self.input_width = 64
        self.num_trains, self.num_vals = 0, 0

        self.cars_train_path = os.path.join('Data', self.dataset_name, 'train')
        self.cars_val_path = os.path.join('Data', self.dataset_name, 'val')
        self._load_cars()
        self.batch_number = 0
    ...
    def _load_cars(self):
        ...
        if self.flags.training == True:
            self.train_data = utils.all_files_under(self.cars_train_path)
            self.num_trains = len(self.train_data)

        self.val_data = utils.all_files_under(self.cars_val_path)
        self.num_vals = len(self.val_data)
        print('Load {} dataset SUCCESS!'.format(self.dataset_name))
    ...
    def val_next_batch(self, batch_size):
        #batch_paths = np.random.choice(self.val_data, batch_size, replace=False)
        batch_paths = self.val_data[self.batch_number:self.batch_number + batch_size]
        self.batch_number += 1
        batch_imgs = [utils.load_data(batch_path, input_height=self.input_height, input_width=self.input_width)
                      for batch_path in batch_paths]
...
def Dataset(flags, dataset_name):
    if dataset_name == 'celebA':
        ...
        return Cars(flags, dataset_name)
    else:
