...
### Train the network on CIFAR-10 or the Stanford cars dataset
#### Preparation
- Download CIFAR-10 dataset from [here](https://www.cs.toronto.edu/~kriz/cifar.html)
- The Stanford cars dataset is already uploaded in the repository in the same format as the CIFAR-10 dataset
...
```
python inception_cifar.py --train --lr LEARNING_RATE --bsize BATCH_SIZE --keep_prob KEEP_PROB --maxepoch NUM_EPOCHS --num_train_batches 1 --optimizer adam
```
- The optimizer can be changed to SGD
