...
class CIFAR(DataFlow):
    def __init__(self,
        ...
        if data_type == 'train':
            self._file_list = [os.path.join(data_dir, 'data_batch_{}'.format(i)) for i in range(1, num_train_batches + 1)]
        else:
...
def unpickle(file):
    ...
    image = np.squeeze(dict[b'data'])
    labels = dict[b'labels']

    image_size = np.int(image.shape[1]/3)
    image_width = np.sqrt(image_size).astype(np.int)

    r = image[:,:image_size].reshape(-1,image_width,image_width)
    g = image[:,image_size: 2*image_size].reshape(-1,image_width,image_width)
    b = image[:,2*image_size:].reshape(-1,image_width,image_width)
