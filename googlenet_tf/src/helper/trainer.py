...
def display(output_file,
    ...
    with open(output_file,'a') as file:
        ...
        for val, name in zip(scaler_sum_list, name_list):
            file.write(' {}: {:.4f}'.format(name, val * 1. / step))
...
class Trainer(object):
    def __init__(self, train_model, valid_model, train_data, output_file, init_lr=1e-3):
        ...
        self._history_dict = {'epoch':[], 'loss': [], 'acc': [], 'val_loss': [], 'val_acc': []}
    ...
    def train_epoch(self, sess, keep_prob=1., summary_writer=None):
        ...
        with open(self._output_file,'a') as file:
            file.write('==== epoch: {}, lr:{} ===='.format(cur_epoch, self._lr))
            file.write('\n')
        ...
        self._history_dict['epoch'].append(cur_epoch)
        self._history_dict['loss'].append(loss_sum*1./step)
        self._history_dict['acc'].append(acc_sum*1./step)
    ...
    def valid_epoch(self, sess, dataflow, summary_writer=None):
        ...
        self._history_dict['val_loss'].append(loss_sum*1./step)
        self._history_dict['val_acc'].append(acc_sum*1./step)
