...
class GoogLeNet(BaseModel):
    ...
    def __init__(self, n_channel, n_class, optimizer = 'adam', pre_trained_path=None,
                 bn=False, wd=0, conv_trainable=False, fc_trainable=True,
    ...
    def _create_variable_input(self):
        self.z = tf.get_variable(dtype=tf.float32,
                      trainable=True, initializer=tf.random.uniform([1, 64, 64, self._n_channel],-1,1), name='z')
        self.image = 255*(tf.tanh(self.z) + 1)/2
        #self.image = tf.get_variable(dtype=tf.float32,
        #              trainable=True, initializer=tf.random.uniform([1, 64, 64, self._n_channel],0,255), name='image')
        self.label = tf.placeholder(tf.int64, [None], 'label')
        self.keep_prob = 1.
    ...
    def create_variable_model(self):
        self.set_is_training(is_training=False)
        self._create_variable_input()
        if self._sub_imagenet_mean:
            net_input = module.sub_rgb2bgr_mean(self.image)
        else:
            net_input = self.image
    ...
    def create_inpainting_model(self, model_input):
        self.set_is_training(is_training=False)
        self.label = tf.placeholder(tf.int64, [None], 'label')
        self.keep_prob = 1.
        if self._sub_imagenet_mean:
            net_input = module.sub_rgb2bgr_mean(model_input)
        else:
    ...
    def _get_optimizer(self):
        if self._optimizer == 'adam':
            ...
            return tf.train.MomentumOptimizer(self.lr, momentum = 0.9, use_nesterov = True)
        else:
            sys.exit('Optimizer has to be either adam or SGD')
