...
DATA_PATH = '../data/cars/'
SAVE_PATH = './examples/callbacks_11'
PRETRINED_PATH = '../data/googlenet.npy'
n_class = 196
IM_PATH = '../data/cars_dataset_reduced/'
IM_PATH = '../data/'
...
def get_args():
    ...
    parser.add_argument('--num_train_batches', type=int, default=1,
...
def train():
    ...
    train_data, valid_data = loader.load_cifar(
        cifar_path=DATA_PATH, batch_size=FLAGS.bsize, subtract_mean=True, num_train_batches = FLAGS.num_train_batches)
    ...
    train_model = GoogLeNet_cifar(
        n_channel=3, n_class=n_class, pre_trained_path=pre_trained_path, optimizer = FLAGS.optimizer,
    ...
    trainer = Trainer(train_model, valid_model, train_data, output_file = os.path.join(dir_callbacks,'out.txt'),init_lr=FLAGS.lr)
    ...
    with tf.Session() as sess:
        ...
        for epoch_id in range(FLAGS.maxepoch):
            ...
            history_dict = trainer.history_dict()
            pickle.dump(history_dict,open(os.path.join(dir_callbacks,'history_dict'),'wb'))
        saver.save(sess, '{}inception-cifar-epoch-{}'.format(SAVE_PATH, epoch_id))
        writer.close()
...
if __name__ == "__main__":
    ...
    if FLAGS.train:
        dirs = os.listdir()
        callbacks_id = np.array([item.replace('callbacks_','') for item  in dirs if item.find('callbacks_') > -1]).astype(np.int)
        if len(callbacks_id) == 0:
            dir_callbacks = 'callbacks_1'
        else:
            dir_callbacks = 'callbacks_' + str(max(callbacks_id) + 1);
        os.makedirs(dir_callbacks)

        SAVE_PATH = dir_callbacks + '/'

        with open(dir_callbacks + '/info.txt','w') as file:
            file.write('Finetuning: ' + str(FLAGS.finetune) + '\n')
            file.write('Optimizer: ' + FLAGS.optimizer + ', lr = ' + str(FLAGS.lr) + '\n')
            file.write('Keep prob: ' + str(FLAGS.keep_prob) + '\n')
            file.write('Batch size: ' + str(FLAGS.bsize) + '\n')
        train()
